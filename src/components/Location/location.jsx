...
const Location = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        setSuccessMessage('Location data updated successfully!');
      } else {
        response = await axios.post('http://localhost:5000/api/location', form, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
        setSuccessMessage('Location data created successfully!');
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setSuccessMessage('Location data deleted successfully!');
      fetchLocationData();
    } catch (err) {
              ...
              {locationData && (
                <tr className="hover:bg-gray-50 transition-colors">
                 <td className="px-5 py-5 border-b border-gray-200 text-sm">
                  ...
                  <td className="px-5 py-5 border-b border-gray-200 text-sm text-gray-700">{locationData.date}</td>
                  ...
                  <td className="px-5 py-5 border-b border-gray-200 text-sm">
                    <div className="flex gap-2">
                      <button onClick={() => handleEdit(locationData)} className="bg-blue-500 hover:bg-blue-600 text-white font-semibold px-3 py-1 rounded-md transition-colors duration-200 flex items-center gap-1 shadow-md">
                        <Edit className="h-4 w-4" /> <span className="hidden sm:inline">Edit</span>
                      </button>
                      <button onClick={() => handleDelete(locationData._id)} className="bg-red-500 hover:bg-red-600 text-white font-semibold px-3 py-1 rounded-md transition-colors duration-200 flex items-center gap-1 shadow-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="relative" onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
              ...
              <div className={`mt-1 p-3 border rounded-md w-full flex items-center justify-center transition-colors duration-200 border-dashed border-gray-300 bg-white text-gray-500 ${dragActive ? 'border-blue-500 bg-blue-50' : ''}`}>
                {formData.image ? (
              ...
              {formData.image && (
                <div className="mt-2">
                  <img src={formData.image instanceof File ? URL.createObjectURL(formData.image) : formData.image} alt="Uploaded" className="h-20 rounded-md shadow-sm border border-gray-200" />
                </div>
              )}
            </div>
            <div className="col-span-full md:col-span-1">
              <label className="text-sm font-medium text-gray-700 block mb-1">Description <span className="text-red-500">*</span></label>
              <textarea name="description" value={formData.description || ''} onChange={handleChange} className="mt-1 w-full rounded-md border border-gray-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent bg-white text-gray-900 placeholder:text-gray-400 text-sm" placeholder="Enter description" rows={3} />
            </div>
            <div className="col-span-full md:col-span-1">
              <label className="text-sm font-medium text-gray-700 block mb-1">Sentence 1</label>
              <input type="text" name="sentence1" value={formData.sentence1 || ''} onChange={handleChange} className="mt-1 w-full rounded-md border border-gray-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent bg-white text-gray-900 placeholder:text-gray-400 text-sm" placeholder="Enter sentence 1" />
            </div>
            <div className="col-span-full md:col-span-1">
              <label className="text-sm font-medium text-gray-700 block mb-1">Sentence 2</label>
              <input type="text" name="sentence2" value={formData.sentence2 || ''} onChange={handleChange} className="mt-1 w-full rounded-md border border-gray-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent bg-white text-gray-900 placeholder:text-gray-400 text-sm" placeholder="Enter sentence 2" />
            </div>
          </div>
          <div className="flex justify-end gap-3 mt-6">
            <button variant="outline" onClick={() => { setShowForm(false); resetForm(); }} className="bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold px-4 py-2 rounded-md transition-colors duration-200 shadow-md">
              <X className="mr-2 h-4 w-4" /> Cancel
            </button>
            <button type="submit" className="bg-red-600 hover:bg-red-700 text-white font-semibold px-4 py-2 rounded-md transition-colors duration-200 shadow-md">
  ...
  return (
    <div className="p-6">
      ...
      <div className="mt-4 flex justify-end">
        {!showForm && (
          <button onClick={() => setShowForm(true)} className="bg-green-500 hover:bg-green-600 text-white font-semibold px-4 py-2 rounded-md transition-colors duration-200 shadow-md">
