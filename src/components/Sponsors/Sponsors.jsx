...
import { MoreVertical, Trash2, Edit, ChevronDown, Upload, Image } from "react-feather";
...
const SponsorsPage = () => {
  ...
  const handleAddSponsor = async () => {
    const { companyName, description, logos, level } = newSponsor;
    if (!companyName || !description || logos.length !== 2) return alert("Fill all fields and upload two logo images.");
  ...
  const handleLogoChange = (acceptedFiles) => {
    setNewSponsor({ ...newSponsor, logos: acceptedFiles });
  };
      ...
      {(isAdding || isEditing) && (
        <div className="bg-white p-6 rounded-lg shadow-lg mb-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              <label className="block text-sm text-gray-700">Upload Two Logo Images</label>
              <Dropzone onDrop={handleLogoChange} accept="image/*" multiple={true} maxFiles={2}>
              ...
              <div className="mt-3 flex space-x-2">
                {newSponsor.logos.map((logo, index) => (
